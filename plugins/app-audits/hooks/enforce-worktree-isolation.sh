#!/usr/bin/env bash
# PreToolUse hook — refuses any Agent dispatch of app-audits:issue-worker that
...
[[ "$subagent" != "app-audits:issue-worker" ]] && exit 0
...
cat >&2 <<'EOF'
BLOCKED by app-audits/hooks/enforce-worktree-isolation.sh.

You dispatched subagent_type "app-audits:issue-worker" without
...
This rule is documented in plugins/app-audits/commands/do-work.md
