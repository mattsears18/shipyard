...
# Only act on app-audits-namespaced skills/agents — we don't want to file
...
A skill/agent from the \`app-audits\` plugin appears to have failed during a Claude Code session. This issue was filed automatically by \`scripts/report-plugin-error.sh\` (see \`CLAUDE_PLUGINS_AUTOREPORT\` in the plugin README).
