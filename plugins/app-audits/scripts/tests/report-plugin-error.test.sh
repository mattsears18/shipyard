...
out=$(echo '{"tool_name":"Agent","tool_input":{"subagent_type":"app-audits:issue-worker"},"tool_response":{"is_error":true,"error":"boom"}}' \
...
out=$(run_helper '{"tool_name":"Agent","tool_input":{"subagent_type":"app-audits:issue-worker"},"tool_response":{"content":"all good"}}')
assert_equals "$out" "" "success payload → no filing"

# Explicit is_error → file.
out=$(run_helper '{"tool_name":"Agent","tool_input":{"subagent_type":"app-audits:issue-worker","prompt":"work issue 1"},"tool_response":{"is_error":true,"content":"Error: tool crashed"}}')
assert_contains "$out" '"title"' "is_error=true → dry-run emits filing"
assert_contains "$out" 'app-audits:issue-worker' "filing carries the subagent name"

# Error: marker in output → file.
out=$(run_helper '{"tool_name":"Agent","tool_input":{"subagent_type":"app-audits:lighthouse-auditor"},"tool_response":{"content":"running…\nError: timed out fetching report"}}')
assert_contains "$out" "app-audits:lighthouse-auditor" "Error: marker triggers filing"

# blocked: marker.
out=$(run_helper '{"tool_name":"Agent","tool_input":{"subagent_type":"app-audits:web-ux-auditor"},"tool_response":{"content":"blocked: ambiguous acceptance criteria"}}')
assert_contains "$out" "app-audits:web-ux-auditor" "blocked: marker triggers filing"

# --------------------------------------------------------------------------
echo "== Namespace filter"
# --------------------------------------------------------------------------

# Non-app-audits subagent → ignored.
out=$(run_helper '{"tool_name":"Agent","tool_input":{"subagent_type":"some-other-plugin:thing"},"tool_response":{"is_error":true,"error":"nope"}}')
assert_equals "$out" "" "non-app-audits subagent → ignored"
...
payload='{"tool_name":"Agent","tool_input":{"subagent_type":"app-audits:issue-worker","prompt":"use token ghp_aBcDeFgHiJkLmNoPqRsTuVwXyZ1234567890"},"tool_response":{"is_error":true,"error":"Error: auth failed with key sk-aBcDeFgHiJkLmNoPqRsTuVwXyZ12345 at /Users/secretuser/.config/foo"}}'
...
# AWS access key
payload='{"tool_name":"Agent","tool_input":{"subagent_type":"app-audits:security-auditor"},"tool_response":{"is_error":true,"error":"Error: aws auth: AKIAIOSFODNN7EXAMPLE failed"}}'
out=$(run_helper "$payload")
assert_not_contains "$out" "AKIAIOSFODNN7EXAMPLE" "AWS access key scrubbed"

# Bearer token
payload='{"tool_name":"Agent","tool_input":{"subagent_type":"app-audits:security-auditor"},"tool_response":{"is_error":true,"error":"Error: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.payload.sig"}}'
out=$(run_helper "$payload")
...
payload='{"hook_event_name":"PostToolUse","tool_name":"Agent","tool_input":{"subagent_type":"app-audits:issue-worker","prompt":"work issue 22","description":"do work"},"tool_response":{"is_error":true,"error":"Error: gh api 404"}}'
out=$(run_helper "$payload")
...
assert_contains "$body" "autoreport-key=app-audits:issue-worker" "body carries de-dup signature"
...
assert_contains "$title" "app-audits:issue-worker" "title carries the subagent name"
...
p1='{"tool_name":"Agent","tool_input":{"subagent_type":"app-audits:lighthouse-auditor"},"tool_response":{"is_error":true,"error":"Error: lighthouse timed out at 30000ms"}}'
p2='{"tool_name":"Agent","tool_input":{"subagent_type":"app-audits:lighthouse-auditor"},"tool_response":{"is_error":true,"error":"Error: lighthouse timed out at 60000ms"}}'
...
p3='{"tool_name":"Agent","tool_input":{"subagent_type":"app-audits:web-ux-auditor"},"tool_response":{"is_error":true,"error":"Error: lighthouse timed out at 30000ms"}}'
...
print(json.dumps({
    'tool_name':'Agent',
    'tool_input':{'subagent_type':'app-audits:issue-worker'},
