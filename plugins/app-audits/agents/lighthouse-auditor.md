...
Use the `app-audits:audit-rubrics` skill for grouping rules. For Lighthouse specifically:
...
Use the `app-audits:filing-github-issues` skill for the filing conventions (title prefixes, label discovery, duplicate search, body template, HEREDOC pattern).
