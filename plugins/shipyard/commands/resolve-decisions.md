...
This is the **mutating sibling** of [`/shipyard:my-turn`](./my-turn.md). `/my-turn` is read-only: it *surfaces* the highest-leverage human-blocked item and, when that item is decision-gated, *offers* to hand off to this command (see [`/my-turn`'s decision-gated offer](./my-turn.md#decision-gated-handoff-offer)). The walkthrough and the record-and-unblock mutation live here, deliberately separated so `/my-turn`'s clean read-only contract stays intact ([#566](https://github.com/mattsears18/shipyard/issues/566)). `/my-turn` *surfaces and elicits*; `/resolve-decisions` *walks, records, and unblocks*.
