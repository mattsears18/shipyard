...
**This formalizes what the spec already assumes operationally — it doesn't introduce new behavior.** `dispatch_denials` and `operator_denials` (see [`do-work.md`'s Orchestrator state](./do-work.md#orchestrator-state)), the classifier-response branches in [`do-work/operate.md`](./do-work/operate.md#operator-action-denied-by-the-harness-permission-classifier-746) and [`do-work/dispatch-rules.md`](./do-work/dispatch-rules.md#dispatch-denied-by-the-harness-permission-classifier-718), and every worker's `shipyard:worker-preamble` § "After a classifier denial" contract all exist *because* the Auto-mode classifier is expected to be live and to occasionally deny a call. Running under full bypass instead doesn't break any of that machinery, but it silently defeats the safety property it was built for — every one of those branches becomes dead code that never fires, because there's no classifier there to deny anything.
