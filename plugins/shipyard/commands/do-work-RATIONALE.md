...
The fix layers an explicit declaration on top: callers export `SHIPYARD_ORCHESTRATOR_PID=<N>` (or pass `--orchestrator-pid <N>` to `classify-lock`) and a PID match short-circuits to `self-ancestor` without consulting the ancestor chain. The orchestrator-side call sites in [`commands/do-work/setup.md`](commands/do-work/setup.md), [`steady-state.md`](commands/do-work/steady-state.md), and [`cleanup-summary.md`](commands/do-work/cleanup-summary.md) auto-detect the orchestrator's PID via the new `worktree-reap.sh detect-orchestrator-pid` helper (which walks the ancestor chain looking for a `comm == claude` process and emits its PID). When detection succeeds the env var carries an authoritative value; when it fails (e.g. binary renamed), classify-lock falls back to the ancestor-walk path from #138 — same behavior as before. The safety property holds: a wrong env-var value can only short-circuit when the lock PID *exactly* matches the env-var value AND that PID is alive — both gates fire after `pid_alive` confirms liveness, so a stale orchestrator PID from a previous session can't wrongly mark a live peer's lock as self-ancestor.
