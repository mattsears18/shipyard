...
## Step 3d — why the `blocked:ci` / `blocked:agent-hard` sweeps have different shapes

The asymmetry between the two sweeps is deliberate: `blocked:ci`'s premise is mechanical ("no new commits since the label was applied"), so the timestamp comparison is the right tool. `blocked:agent-hard`'s premise is referential ("a specific other issue must close first"), so a body-reference scan is the right tool. Same shape, different signal. (Pre-#300 the second sweep operated on a single `blocked:agent` label — the rationale stayed the same when the label split into `-hard` / `-soft`; the referential sweep targets `-hard` exclusively, since `-soft` auto-clears at next-session start regardless of referenced-blocker state.)
...
**Why split into `-hard` and `-soft`.** Of the 7 worker bail strings in `agents/issue-worker/issue-work.md`, only 3 represent genuine "needs human review" refusals (the prompt-injection / out-of-scope-action triplet — security gates that must not auto-retry). The other 4 are speculative judgments: a different worker reading the same body with slightly different priors could plausibly succeed. Treating both classes identically (uniform `blocked:agent` → permanent hide) was the structural flaw. The split surfaces the distinction as a class-typed label:

- `blocked:agent-hard` keeps the pre-#300 semantics: excluded from step 4's workable filter, auto-cleared only when referenced blockers close (step 3d.2 sub-sweep a / step A.5 mid-session sweep). Conservative — these are the bails that genuinely need a human eye.
- `blocked:agent-soft` is documented as "subjective bail; auto-cleared at next session." Step 4's filter does NOT exclude it. Step 3d.2 sub-sweep c removes it at every session start. Within a session, `session_blocked_soft` + `blocked_agent.soft_retry_minutes` (default 30 min) prevents tight retry loops while still allowing the issue to be re-considered after the rest of the session has moved forward.
...
**Legacy `blocked:agent` migration.** Pre-#300 issues still carry the bare label. Sub-sweep b (in step 3d.2) migrates each one to `blocked:agent-hard` (the conservative default — the pre-#300 worker that stamped it didn't classify, so the bail reason could have been any of the 7 strings). A maintainer can manually swap the label to `-soft` if they remember the bail was subjective; the migration comment explains the path. The legacy label stays registered (the `gh label create` call still fires) so a future audit can confirm zero open issues carry it before deletion. Eventually (a few sessions with zero legacy hits) the label and the migration code can be dropped.

## Step A.5 — why mid-session blocked-issue re-evaluation was added

**The gap.** Step 3d.2 sub-sweep a at session-start auto-clears `blocked:agent-hard` labels when all `Blocked by #N` references resolve to CLOSED/MERGED. But it only runs once. If a session ships PR #X which closes issue #N, any issue that had `Blocked by #N` in its body stays hidden in `raw_backlog`'s exclusion set until the *next* `/do-work` invocation — even though the blocker is now gone. The cost was manual intervention: the user had to edit the blocked issue's body to remove the `Blocked by` line and re-add it to the backlog. See [#245](https://github.com/mattsears18/shipyard/issues/245) for the session reproducer. (Per [#300](https://github.com/mattsears18/shipyard/issues/300), `blocked:agent-soft` is out of scope here — soft labels auto-clear at next-session backlog fetch regardless of referenced-blocker state.)

**Why fire on `shipped #<N> via PR #<M>` and not on a periodic timer.** A timer would mean running the search on every step-D refresh regardless of whether any blockers changed. The `shipped` return is the exact event that causes a blocker to close — firing precisely on that signal means zero false-positive sweeps on sessions where no blocked issues exist. The cost is one `gh pr view --json closingIssuesReferences` call per shipped PR plus one `gh issue list --label blocked:agent-hard --search "Blocked by #N"` per closed issue — negligible compared to the dispatch cost of the shipped worker.
...
**False-positive handling.** Same as step 3d.2 sub-sweep a: the issue might have secondary gates ("also wait for Phase B"). A false positive is recoverable — the issue worker returns `blocked` after scoping and the orchestrator applies `blocked:agent-hard` again. The cost of one extra worker invocation on a false positive is far lower than leaving a genuinely-unblocked issue buried for an entire session.
