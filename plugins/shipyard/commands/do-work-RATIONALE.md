...
**Why the detector is structured for extensibility.** The current implementation has one detector (`.github/workflows/` mention), but the spec section is structured as a numbered sub-section under "Pre-scope orchestrator-side detectors" precisely so additional detectors slot in without re-organizing the surrounding handler. Future candidates: bodies that propose new GitHub Actions secrets, bodies that propose changes to `.github/CODEOWNERS`, bodies that propose changes to branch-protection rules via a config file. Each new detector follows the same shape: a pure body string-match → a synthesized deferred entry with `provenance: "orchestrator-judgment"` and a structured `evidence_pointer` matching the validator's per-class shape. The pattern is intentionally small; the section grows by addition rather than rewrite.
