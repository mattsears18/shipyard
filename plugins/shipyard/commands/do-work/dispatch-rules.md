...
**Per-mode model routing.** Every `mode:`-driven worker dispatches through the [`Workflow`-tool substrate](#workflow-substrate-dispatch--the-dispatch-mechanism-for-every-worker-mode-791) — the mode itself selects the prompt builder inside [`workflows/do-work-dispatch.workflow.js`](../../workflows/do-work-dispatch.workflow.js), and the only per-mode dispatch *parameter* the orchestrator resolves here is the model. Model tiering is **role-based** ([#784](https://github.com/mattsears18/shipyard/issues/784)): implementation defaults to the cheap **Sonnet 5** agent-runner, the mechanical fix modes pin **Haiku** (cheaper still — mostly pattern-matching against failing logs), and the strong, harder-to-fool **Opus 4.8** tier is reserved for the verify gate, where the highest-stakes judgment earns its price. Vantage's cost analysis makes tiering a top cost lever (~50–70% claimed savings vs all-Opus) *and* a quality win (Opus 4.8 is ~4× less likely than 4.7 to silently pass flawed code). See [#157](https://github.com/mattsears18/shipyard/issues/157) for the original cost rationale and [#784](https://github.com/mattsears18/shipyard/issues/784) for the tier-by-role refresh.

| `mode:`                  | Per-mode spec (the worker loads this)                          | Model (effective **default**) | Reason for the model choice                                       |
|--------------------------|-----------------------------------------------------------------|---------------------|-------------------------------------------------------------------|
| `issue-work`             | [`issue-worker/issue-work.md`](../../agents/issue-worker/issue-work.md)                       | `sonnet` (Sonnet 5 — `models.issue_work` default) | Code authorship, test design, PR composition — Sonnet 5 is the cheap 1M-context implementation tier. |
| `fix-checks-only`        | [`issue-worker/fix-checks-only.md`](../../agents/issue-worker/fix-checks-only.md)             | `haiku`             | Pattern-match the failing log, apply targeted fix.                |
| `fix-rebase`             | [`issue-worker/fix-rebase.md`](../../agents/issue-worker/fix-rebase.md)                       | `haiku`             | Git mechanics — fetch + rebase + force-with-lease.                |
| `fix-main-ci`            | [`issue-worker/fix-main-ci.md`](../../agents/issue-worker/fix-main-ci.md)                     | `sonnet`            | No PR context to anchor; broader investigation than fix-checks.   |
| `fix-failing-prs-batch`  | [`issue-worker/fix-failing-prs-batch.md`](../../agents/issue-worker/fix-failing-prs-batch.md) | `sonnet`            | Cross-PR pattern-spotting across ≤5 representative failures.      |
| `investigate`            | [`issue-worker/investigate.md`](../../agents/issue-worker/investigate.md)                     | `sonnet`            | Investigate untriaged/bot-authored crash reports; disposition into binary backlog. |
| `spike`                  | [`issue-worker/spike.md`](../../agents/issue-worker/spike.md)                                 | default (session model / Opus) — Fable 5 opt-in via `models.spike` | Feasibility judgment + design-doc authorship — same reasoning tier as issue-work, no cheaper pin. |

Every value in the Model column is the **default**, not the last word — the merged config's `models.<mode>` overrides it per the model-resolution rule below. For `issue-work`, `sonnet` is the **effective** default via the built-in `models.issue_work` config value. The **verify gate** is dispatched *by the issue-work worker* (not by this orchestrator table) and pins **Opus 4.8** via [`verify-worker.md`](../../agents/verify-worker.md)'s frontmatter, overridable through `models.verify` — see [`issue-worker/verify.md`](../../agents/issue-worker/verify.md).

**The seven `shipyard:*-worker` mode shims are no longer a dispatch target ([#791](https://github.com/mattsears18/shipyard/issues/791)).** Before the [#782](https://github.com/mattsears18/shipyard/issues/782) Dynamic Workflows migration, this table's second column named an `Agent`-tool `subagent_type` per mode and the orchestrator dispatched each worker through the `Agent` tool. That path is retired: the `Workflow` tool's `agent()` primitive takes no `subagent_type`, so a worker's mode-specific behavior is now carried entirely by the prompt the workflow script builds (whose first instructions are "load `shipyard:worker-preamble`, then `agents/issue-worker/<mode>.md`"). The shim agent files under [`agents/`](../../agents/) are retained — they remain valid definitions a human can dispatch by hand, they still forward to the same per-mode specs, and their `model:` frontmatter still feeds the pricing-coverage test — but the orchestrator never names them. **When in doubt about a mode's behavioral contract, read the per-mode file; it is the single source of truth under either dispatch shape.**

**`shipyard:decompose-worker` is intentionally absent from this table.** It doesn't take a `mode:` value at all, isn't reached through this per-issue routing tree, and is still dispatched via the **`Agent` tool** (deliberately without `isolation: "worktree"` — it never touches code) — see [Wiring `shipyard:decompose-worker` into the existing inline auto-decompose dispatch](#wiring-shipyarddecompose-worker-into-the-existing-inline-auto-decompose-dispatch-774) below for where it's actually invoked. The `Workflow` substrate governs the seven `mode:`-driven worker modes only; it did not absorb every `Agent` dispatch shipyard makes.

**Per-dispatch model resolution — honor `models.<mode>` ([#727](https://github.com/mattsears18/shipyard/issues/727)).** The plugin's built-in per-mode defaults are plugin-owned, so a consumer repo cannot edit them in place — which is precisely what the `models.*` config block exists for. Until #727 that block was a **dead surface**: schema-defined, settable via `/shipyard:config set models.issue_work <id>`, documented in CLAUDE.md, covered by tests — and read by nothing, so a user who set `models.issue_work: claude-sonnet-4-6` to cut spend silently kept dispatching on the session model, with no warning anywhere. **Every** dispatch in the table above — step 7's initial pool fill and step C's replacement dispatch alike — now resolves the mode's model from the merged config and passes it through to the worker's `agent()` stage:
...
- **`dispatch_model` non-empty** (`opus` / `sonnet` / `haiku` / `fable`) → set it as the work unit's `model` field in the `Workflow` call's `args.issues[]` payload (the script forwards it to that unit's `agent()` stage as its `model` option).
- **`dispatch_model` empty** → **omit** the `model` field entirely so the workflow runtime's own default applies. Empty means "no override": the key is unset in every config layer, the config was unreadable, or the configured id matched no known model family (the script warns on stderr in that last case).

**This is a script call, not a rule to re-derive** — [`scripts/resolve-dispatch-model.sh`](../../scripts/resolve-dispatch-model.sh) is the single executable source of truth, for the same anti-drift reason as [`detect-ungated-admin-direct-merge.sh`](../../scripts/detect-ungated-admin-direct-merge.sh) (#716). In particular, do NOT pass the raw config value through: the `model` option is an **enum** (`opus` | `sonnet` | `haiku` | `fable`), while the config (and the pricing table) speak in concrete ids like `claude-sonnet-4-6`; the script maps the id onto its family alias, and a raw id would fail input validation. The built-in defaults (`shipyard-config.sh`) already mirror the per-mode tiers family-for-family, so on a repo that sets no `models.*` override the resolution is behaviorally a no-op — it only diverges when a repo (or the user-global layer) deliberately asks for a different tier.

**Don't fail a dispatch on a model-resolution problem.** The resolver's posture is fail-open: an unreadable config or a typo'd model id resolves to empty, and the dispatch proceeds on the runtime default. A missing override must never block work.

**Every dispatch in the table above MUST run in an isolated worktree the orchestrator pre-provisions itself, passed as the work unit's `worktreePath` ([#791](https://github.com/mattsears18/shipyard/issues/791)).** The Dynamic Workflows `agent()` primitive documents no isolation/worktree option, so — unlike the retired `Agent`-tool path, where the harness provisioned and cwd-pinned the worktree in response to `isolation: "worktree"` — nothing in the runtime closes this gap for you. The orchestrator, which still has full shell access, runs `git worktree add` before the `Workflow` call and passes the resulting absolute path in; the built prompt's very first instruction is a `cd` into it plus the same git-dir-vs-git-common-dir verification `shipyard:worker-preamble`'s step-0 fail-fast uses. See [step 1 of the substrate section](#workflow-substrate-dispatch--the-dispatch-mechanism-for-every-worker-mode-791) for the per-mode `git worktree add` invocations. The [`enforce-worktree-isolation.sh`](../../hooks/enforce-worktree-isolation.sh) `PreToolUse` hook hard-fails any `Workflow` dispatch of `do-work-dispatch.workflow.js` carrying a work unit with no `worktreePath` — the direct successor to the guard that previously hard-failed an `Agent` dispatch missing `isolation: "worktree"` (#293 / #514 / #774). That hook still guards the `Agent` shape too, for the worktree-isolated agents that are *not* `mode:`-driven workers (chiefly `shipyard:verify-worker`) and for hand dispatches of the retained mode shims.

When filling a slot, walk this decision tree:

1. **`divert_queue` non-empty?** → pop the front entry. Path-collision rules don't apply (these are synthetic, not file-claimed). Dispatch a worker in the matching mode (`mode:` selects the model per the table above and the prompt builder inside the workflow script). Only one diverted worker per kind can be in flight at a time (step 4.5 / step D enforce this on enqueue).
...
3. **`ready_issues` non-empty?** → scan from the head for the first entry whose `claimed_paths` **don't collide** with any entry in `in_flight`, per the two-tier collision rule below.
   ...
   **This template is the prose spec of the prompt; the workflow script builds it.** The augmentations below (verify-gate, user-feedback preamble, phase-1 slice, next-available-version) are computed **by the orchestrator** and passed as `args.issues[0]` fields; `buildIssueWorkPrompt` in [`do-work-dispatch.workflow.js`](../../workflows/do-work-dispatch.workflow.js) assembles them into the text above. Resolve every augmentation first, then hand the whole set to the `Workflow` call per the [substrate section](#workflow-substrate-dispatch--the-dispatch-mechanism-for-every-worker-mode-791). Keep this template and the script's builder in sync — the template is the reviewable prose, the builder is what actually runs.
...
Dispatch is via a **background `Workflow` call** against [`workflows/do-work-dispatch.workflow.js`](../../workflows/do-work-dispatch.workflow.js): `run_in_background: true`, one work unit in `args.issues`, carrying the candidate's `mode`, the pre-provisioned `worktreePath`, the mode-specific fields, and — when [`resolve-dispatch-model.sh <mode>`](../../scripts/resolve-dispatch-model.sh) returns non-empty — the `model` it resolved from `models.<mode>` (omit the field when it returns empty, per the per-dispatch model-resolution rule above). The [substrate section below](#workflow-substrate-dispatch--the-dispatch-mechanism-for-every-worker-mode-791) is the complete call-site walkthrough. The harness will notify you on completion — that drives the next iteration of the steady-state loop.
...
## Workflow-substrate dispatch — the dispatch mechanism for every worker mode (#791)

The decision tree above (steps 1–6) decides **which** candidate to run and computes every mode-specific augmentation (divert-queue priority, spike-shape detection, collision tiering, priority scoring, author-trust resolution, verify-gate / user-feedback / phase-1-slice / version-coordination / triage-policy / decompose-cap). This section is **how** that candidate is actually run — the one and only dispatch mechanism, for all seven `mode:` values.

**There is no substrate flag and no alternate path ([#791](https://github.com/mattsears18/shipyard/issues/791), the final phase of the [#782](https://github.com/mattsears18/shipyard/issues/782) epic).** The `#782` migration ran in five phases: #787 committed the script as inert scaffolding; #788 wired `issue-work`; #789 wired the remaining six modes; #790 flipped the built-in `dispatch.substrate` default from `"agent"` to `"workflow"` (the 3.0.0 major bump) while retaining the legacy hand-rolled `Agent`-tool orchestrator for one release as an instant-revert override; **#791 retired that legacy path and deleted the `dispatch.substrate` knob** (see the [migration note](#migration--the-retired-dispatchsubstrate-knob-791) at the end of this section). Every `mode:`-driven dispatch now follows the five steps below unconditionally — there is nothing to read from config, nothing to branch on, and no fallback to fall back to.

1. **Pre-provision the isolated worktree yourself — the `Workflow` tool's `agent()` primitive has no isolation option.** As of the current Dynamic Workflows docs (code.claude.com/docs/en/workflows), `agent()` documents no worktree/isolation/sandboxing option — a Dynamic Workflow script has no filesystem/shell access of its own ("Agents read, write, and run commands. The script coordinates the agents") and nothing in its documented `agent(prompt, opts)` option surface (`label`/`model`/`schema`) closes that gap. (The retired `Agent` tool's `isolation: "worktree"` parameter had the harness auto-provision and cwd-pin the worktree; nothing in the workflow runtime replaces it.) The orchestrator session — unlike the workflow script — still has full shell access at this point, so it closes the gap itself, the same way the harness would have. **This step is not optional**: [`enforce-worktree-isolation.sh`](../../hooks/enforce-worktree-isolation.sh) hard-fails a `Workflow` dispatch of this script carrying a work unit with no `worktreePath`, and the script's own prompt builders emit a `CALLER BUG` instruction telling the worker to return `blocked` rather than operate from an unpinned cwd. **The exact invocation depends on the mode's branch shape:**
...
### Migration — the retired `dispatch.substrate` knob (#791)

`dispatch.substrate` is **gone**: removed from the built-in defaults (`scripts/shipyard-config.sh`), removed from [`schemas/shipyard.config.schema.json`](../../schemas/shipyard.config.schema.json), and read by nothing. Because the repo/local config schema is `additionalProperties: false`, a config layer that still carries a `dispatch` object now **fails schema validation** (`/shipyard:config validate` reports it, and `shipyard-config.sh load` exits 70) rather than being silently ignored. This is why [#791](https://github.com/mattsears18/shipyard/issues/791) carries a **major** version bump.

**To migrate:** delete the `dispatch` block from `shipyard.config.json` and/or `.shipyard/config.local.json`. There is nothing to replace it with — the `Workflow` substrate is the only substrate, and it needs no configuration. If you were relying on `dispatch.substrate: "agent"` to work around a `Workflow`-substrate regression, that escape hatch no longer exists: file the regression against [`mattsears18/shipyard`](https://github.com/mattsears18/shipyard/issues) instead.
...
Every branch above assumes the dispatch tool call *happens*. It can also be **refused outright by the harness** — Claude Code's auto-mode permission classifier evaluates the orchestrator's own dispatch (the `Workflow` call for a `mode:`-driven worker, or the `Agent` call for `shipyard:decompose-worker`), and can deny it:
...
**Reap the pre-provisioned worktree on a denial ([#791](https://github.com/mattsears18/shipyard/issues/791)).** Under the `Workflow` substrate the orchestrator creates the worker's worktree *itself*, in [step 1](#workflow-substrate-dispatch--the-dispatch-mechanism-for-every-worker-mode-791), **before** the `Workflow` call — so unlike the retired `Agent`-tool path (where the harness created the worktree only if the dispatch was allowed), a denial leaves a real, empty `agent-workflow-*` worktree and its branch on disk with no worker and no `.in_flight` slot pointing at them. Nothing else will clean them up: every reap path keys off an `.in_flight` slot or a completion. Remove them in the same turn as the denial, before continuing to the next candidate:
