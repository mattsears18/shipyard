...
3. **No label disqualifies.** The candidate carries **none** of:
   - `needs-triage`, `needs-human-review`, `needs-refinement` — these signal the issue isn't ready for any worker, inline or dispatched. (`needs-human-review` now subsumes the former `needs-design` design-gate per [#515](https://github.com/mattsears18/shipyard/issues/515).)
