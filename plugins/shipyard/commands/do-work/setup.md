...
```
{ issue: N, deferred: "<one-paragraph reason the orchestrator should tell the human>", defer_reason_class: "<class>", would_be_dispatchable_as_phase_1_if?: "<one-line description of the unblocked condition>" }
```
...
Scoping-agent prompt instruction: *Default to slicing, not deferring. If the issue is multi-phase, RETURN the smallest dispatchable phase-1 slice as a ready shape with explicit `phase_1_scope` text listing what's in and what's explicitly out of scope (the worker will file the out-of-scope items as follow-up issues, one per phase). Return the deferred shape ONLY when there's no first phase that can ship independently — e.g. every phase depends on an external decision, on infrastructure that isn't provisioned, on a referenced blocker that hasn't resolved. When in doubt — default to ready (with phase_1_scope if multi-phase). When the issue truly can't be sliced, pick a `defer_reason_class` from the five allowed values and, where possible, fill `would_be_dispatchable_as_phase_1_if` with the condition that would unblock the slice — the orchestrator's pre-drain re-validation reads that field to decide whether the unblocking condition has changed.* See [RATIONALE → Deferred shape](../do-work-RATIONALE.md#step-6--why-scope-pre-flight-has-a-deferred-shape).
