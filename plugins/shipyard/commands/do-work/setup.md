...
- **Deferred entries** — first run the **per-class `evidence_pointer` validation** ([#302](https://github.com/mattsears18/shipyard/issues/302)), then either reject the malformed defer or record the valid one.
  ...
  **Evidence validation** (runs before any of the recording steps below):
  ...
  - `human-decision-required` → same speculative-judgment word check as `external-dependency`. Additionally, generic phrases like "needs design review", "needs product input" without a specific decision named (e.g., what copy is being decided, what design surface) are rejected. The structured prefix `Proposes .github/workflows/` is explicitly accepted (this is the shape the [pre-scope `.github/workflows/` detector](#pre-scope-orchestrator-side-detectors-synthetic-defers) synthesizes — the decision being named is whether to accept the workflow proposal, and CI/infrastructure-policy is a valid decision category per the per-class shape table above).
