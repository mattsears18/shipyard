...
- **[Step 3d.2](#3-ensure-label-exists--recover-from-prior-session)** — two sub-sweeps in sequence: legacy `blocked:agent` migration (re-pointed per [#521](https://github.com/mattsears18/shipyard/issues/521) — dependency-wait → no label, else → `needs-human-review`), `blocked:agent-soft` next-session sweep. (Sub-sweep a, the `blocked:agent-hard` referential clear, was deleted in [#521](https://github.com/mattsears18/shipyard/issues/521).) Per-issue blocker-state lookups (sub-sweep b) read through the [`blocker_state` cache](#08-blocker_state-cache-default-on). **Skipped under `--fast`** (the initial label counts still run for advisory reporting — see step 3d.2's `--fast` note).
...
> ```
>
> Save the four counts as `fast_skip_needs_refinement` (refinement candidates deferred), `fast_skip_blocked_ci`, `fast_skip_blocked_agent_soft`, and `fast_skip_blocked_agent_legacy`. Proceed immediately to step 3. (`blocked:agent-hard` was eliminated in [#521](https://github.com/mattsears18/shipyard/issues/521) — no `fast_skip_blocked_agent_hard` count.)
...
The **Auto-cleared this session** block prints when `cleared_blocked_soft > 0` or `migrated_legacy > 0` (= `migrated_legacy_dep + migrated_legacy_review`). Numbers come from step 3d.2. (The former `blocked:agent-hard` cleared/held counters were removed in [#521](https://github.com/mattsears18/shipyard/issues/521) with sub-sweep a.)
...
**Order matters.** Sub-sweeps b → c run in sequence (sub-sweep a was deleted in [#521](https://github.com/mattsears18/shipyard/issues/521)). c runs after b so the soft sweep operates on the post-migration label set — a legacy `blocked:agent` issue migrated to `blocked:agent-soft` by a maintainer mid-window is swept by c on the same pass.
...
- **Deferred entries** — first run the **per-class `evidence_pointer` validation** ([#302](https://github.com/mattsears18/shipyard/issues/302)), then either reject the malformed defer or record the valid one.
  ...
  **Recording path** (when validation passes):
  ...
  1. Post a comment on the issue: `Scope-preflight diagnosis (not auto-fixable as a single worker): <deferred reason>` — and when the agent supplied `would_be_dispatchable_as_phase_1_if`, append a second paragraph: `Phase-1 dispatchable if: <would_be_dispatchable_as_phase_1_if>`. Use `gh issue comment <N> --repo <owner/repo> --body "..."`. If the comment fails (rate limit, permission), log an advisory and continue — don't block the pre-flight pass on a single comment failure. **When `defer_reason_class == "confirmed-non-shippable-as-single-PR"`, prepend the `<!-- do-work-needs-decomposition -->` trigger marker as the comment's literal first line** (issue [#519](https://github.com/mattsears18/shipyard/issues/519)) — that marker is what [`/decompose-epic`'s candidate fetch](../decompose-epic.md#3-fetch-candidates) keys on to identify an epic-decomposition handoff among the broader `needs-human-review` pool (the binary-backlog re-key folded the dedicated `needs-decomposition` label into `needs-human-review` + this body marker). The marker is NOT added for the other four defer classes — they aren't epic handoffs and `/decompose-epic` must not pick them up. Concretely, the comment body for this class is:
  ...
  3. **When `defer_reason_class == "confirmed-non-shippable-as-single-PR"`, apply the `needs-human-review` surfacing label** (issue [#498](https://github.com/mattsears18/shipyard/issues/498); re-keyed from the former dedicated `needs-decomposition` label to `needs-human-review` + the `<!-- do-work-needs-decomposition -->` body marker by [#519](https://github.com/mattsears18/shipyard/issues/519)) — but **ensure-then-label-then-verify**, never a bare `--add-label` that silently depends on [step 3a](#3-ensure-label-exists--recover-from-prior-session)'s best-effort background create having landed (issue [#508](https://github.com/mattsears18/shipyard/issues/508)):
     ...
     if ! gh issue view <N> --repo <owner/repo> --json labels \
       ...
       echo "[scope-preflight] WARNING: #<N> needs-human-review apply did not land — human-handoff surfacing failed; epic will be re-scoped next session"
     fi
     ```

     This is the keystone that converts the silent diagnosis comment into a tracked human-handoff: it exits the issue from **both** loops — `/do-work` stops re-scoping it every session (the label is in the [step 4 dispatch-exclusion set](#4-pull-the-workable-backlog) below), and `/my-turn` surfaces it as a human-blocked item rather than letting the epic accumulate invisibly. Without the label the diagnosis comment's human-handoff never reaches the human queue (re-scoped every session, never surfaced by `/my-turn`). The **`<!-- do-work-needs-decomposition -->` trigger marker** posted on the step-1 diagnosis comment (above) is the discriminator that distinguishes this epic-decomposition handoff from every *other* `needs-human-review` issue, so [`/decompose-epic`](../decompose-epic.md#3-fetch-candidates) can still find exactly the epics to auto-shard — the label alone is no longer a sufficient signal because it's now the shared binary-backlog human-gate. **Split the mutations** — if this defer path also clears the `@me` self-assign, run `--remove-assignee @me` as its **own** `gh issue edit` call, never combined with `--add-label` in one atomic invocation; otherwise a missing-label failure drops the unassign too (issue [#508](https://github.com/mattsears18/shipyard/issues/508)). If the `gh issue edit` fails (rate limit, permission), the read-back warning fires and the diagnosis comment from step 1 is still posted, so the human still has the comment trail (including the trigger marker). **For the other four defer classes** (`external-dependency`, `human-decision-required`, `untrusted-author`, `confirmed-blocker-still-open`) do **not** apply `needs-human-review` here and do **not** stamp the trigger marker — those defers are not epic-decomposition handoffs (the human action is "resolve the blocker / provision the dependency / make the decision," not "slice this into phases"), so they keep the original behavior: no label, the issue stays open with just the diagnosis comment. In all cases: do not close the issue, do not assign to a human — the human reads the comment (and, for the unsliceable case, sees the `needs-human-review` label + the trigger marker) and decides what to do (open a multi-PR plan, escalate, defer to a sprint, etc.).
