...
**`--operate` adds a fifth gate: `operator_queue` must be empty.** When `--operate` is set, the loop has a second work-source the orchestrator drains itself (the [operator phase](./operate.md)). Before declaring termination, the `operator_queue` must also be empty — if it holds any item, do NOT proceed to drain; drain the next item on the main thread (per [operate.md](./operate.md#draining)) and return control. The exception is the degraded path (no browser backend reachable): there the queue can't be drained, so its items are surfaced as hand-backs in the end-of-session summary and do **not** block termination. Without `--operate`, `operator_queue` is always empty and this gate is a no-op.
