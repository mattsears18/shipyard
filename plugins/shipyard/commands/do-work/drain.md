...
**If no detector fires, check the same-session freshness carve-out before dispatching a scope agent.** Skip the scope-agent dispatch and re-validate mechanically when ALL THREE of the following are true (issue [#549](https://github.com/mattsears18/shipyard/issues/549)):

1. **`entry.deferred_at` is within the current session** — i.e. `entry.deferred_at >= session.started_at` (the entry was written by this session's own scope pre-flight, not inherited from a prior session).
2. **`entry.evidence_pointer` re-validates mechanically right now.** Run the same per-class shape check from [setup.md step 6's per-class validator](./setup.md#per-class-evidence-shapes--what-evidence_pointer-must-look-like) against the existing `evidence_pointer`, then run the cheap class-specific ground-truth probe:
...
3. **The issue body has not been amended since the defer** — fetch `gh issue view <N> --repo <owner/repo> --json updatedAt -q .updatedAt` and confirm the returned timestamp precedes `entry.deferred_at`. An amendment between the defer and now means the original scope agent's read is potentially stale, so the carve-out does not apply.

If all three pass → keep the entry in `deferred_issues` as-is. Log: `[pre-drain-revalidate] #<N> same-session freshness carve-out — mechanical re-validation confirmed (class=<entry.defer_reason_class>, evidence=<entry.evidence_pointer>); scope-agent dispatch skipped`. Do NOT bump `deferred_at` (the field records when the original scope agent deferred; the carve-out is not a new defer — it's a confirmation that the original remains valid). Move on to the next entry.
...
- Condition 1 fails: `[pre-drain-revalidate] #<N> freshness carve-out skipped — entry is from a prior session (deferred_at <entry.deferred_at> < session started_at <session.started_at>); dispatching scope agent`
...
**Why the carve-out is safe.** The three conditions together close the gap the full scope-agent re-dispatch is designed to cover. Condition 1 excludes cross-session entries — the justifications for why a fresh scope-agent pass produces new answers ("main may have advanced since", "audit data may have aged", "original agent may have been permissively-conservative on a body that's tractable on second look") are all cross-session arguments; they do not apply to a same-session defer where the original agent ran minutes ago under the same prompt. Condition 2 re-proves the defer's mechanical grounding right now (the blocker is still open, the pointer still passes shape validation) — the exact checks the scope agent would perform if it ran and returned the same deferred shape. Condition 3 catches the only event that would genuinely make the body-read stale within a session: an amendment between the scope agent's read and the pre-drain re-validation. All three together are equivalent to "the scope agent, re-run now, would reach the same conclusion" — so the dispatch adds cost without correctness gain. The #299 drain-on-misjudged-defers protection stays fully intact for cross-session entries (condition 1 fails → scope-agent dispatch runs, same as before) and for entries whose evidence has shifted (condition 2 fails) or whose issue was amended (condition 3 fails).
...
The cost of 5.b is **at most** one scope-agent invocation per scope-agent defer that reaches the pre-drain check — bounded by `|deferred_issues with scope-agent provenance that do NOT qualify for the same-session freshness carve-out|`. Same-session entries that pass the three-condition mechanical check (deferred this session + evidence re-validates + body unchanged) are handled in microseconds with a `gh issue view --json updatedAt` call instead of a full ~40k-token scope-agent dispatch. The full scope-agent re-dispatch still fires for cross-session entries and for same-session entries whose evidence has shifted or whose issue body was amended after the defer. The #299-drain-on-misjudged-defers protection is fully intact for all non-carve-out entries — the alternative (treating scope-agent defers as authoritative ground truth and letting drain fire on a single misjudgment) was the failure mode [#299](https://github.com/mattsears18/shipyard/issues/299) documented: a `mattsears18/lightwork` session that drained on 15+ workable issues because the JIT scope pre-flight at C=1 had returned `deferred` on each of them in sequence. The phase-slicing bias added by [#298](https://github.com/mattsears18/shipyard/issues/298) makes the re-validation more likely to flip an entry to ready — not because the issue's state has changed, but because the slicing-biased prompt actively searches for a phase-1 slice the original conservative pass passed over. That slicing argument is a cross-session argument; it doesn't apply to a same-session defer where the original agent and the re-validator are the same session, same prompt, within the hour (the motivation for [#549](https://github.com/mattsears18/shipyard/issues/549)).
...
2. If no issues were re-admitted (all defers were confirmed still-valid by their respective re-validation): emit the **pre-drain audit banner** and proceed to drain:
   ...
   PRE-DRAIN AUDIT
       ...
       of those, scope-agent-backed: <K> (re-validated: <Ka> scope-agent dispatch, <Kb> same-session mechanical)  orchestrator-judgment: <J> (re-validated)
   ...
   In the `scope-agent-backed` line: `<Ka>` is the count of scope-agent entries that went through the full scope-agent re-dispatch; `<Kb>` is the count that qualified for the same-session freshness carve-out (mechanically re-validated, no scope-agent dispatch). Both are covered by "re-validated" — they're just different re-validation mechanisms. When `<Kb>` is 0 (all entries used full re-dispatch), emit the shorter form `(<Ka> scope-agent dispatch)` or simply `(re-validated via scope agent)` to keep the banner terse.
