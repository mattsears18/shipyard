...
**Output contract.** The subcommand emits, in order: a `candidates: <T>` header (and, under `--dry-run`, a `dry-run: no writes will be performed` line; and, when at least one candidate worktree carries a `node_modules` tree, a `heavy-worktrees: <H> of <T> candidate worktree(s) carry a node_modules tree — removal is slow` line — [#1559](https://github.com/mattsears18/shipyard/issues/1559)'s removal-cost estimate, absent when `<H>` is 0); one `[k/T] <action> <branch> — <why>` progress line per candidate, each emitted **before** that candidate's write; zero or more `failed-pr: <M>` lines (one per PR #333-flagged red PR the worktree loop found already open — fold each `<M>` onto the same `failed_prs` list step 5's failing-PR scan populates); zero or more `stale-assign: <N>` lines (one per issue the row-5 self-assign sweep cleared); zero or more `already-landed: <branch> — <why>` lines (one per candidate the [#1517](https://github.com/mattsears18/shipyard/issues/1517) pre-checks suppressed); zero or more `removal-deferred: <branch>` lines followed by a single `removal-cap-reached: <R> already-landed worktree(s) not removed; re-run or raise --max-removals (current: <N>)` line when the `--max-removals` cap stopped the removals short ([#1559](https://github.com/mattsears18/shipyard/issues/1559)); zero or more `deferred: <branch>` lines followed by a single `cap-reached: <D> candidate(s) remaining, not actioned; re-run or raise --max-prs (current: <N>)` line when the `--max-prs` cap stopped the sweep short; then a trailing `summary: salvaged=<S> abandoned=<A> stale_assigns=<SA> already_landed=<L>` line — read `salvaged_count`/`abandoned_count`/`stale_assigns_count`/`already_landed_count` straight off it. **That summary line's four-field shape is fixed** — neither cap adds a field to it, exactly as `--max-prs`' own deferrals never did; both report through their own `*cap-reached:` line instead. **An `already-landed:` line is a suppression, not a failure** — it means the sweep correctly declined to manufacture a duplicate PR, and a run whose whole candidate set classifies that way is the expected steady state on a squash-merge repo with a long shipped-issue history. **A `cap-reached:` or `removal-cap-reached:` line is normal on a repo with an accumulated backlog** — not an error, and not a reason to retry with a raised cap in the same session; note the count and continue. A `removal-deferred:` candidate is still counted in `already_landed=<L>` and still had its PR suppressed — only its `git worktree remove` waits for a later session. Interleaved with those, the subcommand ALSO prints one of these three literal lines per freshly-created PR — the first is the #1518 default posture; the latter two are reachable only under the explicit `--arm-auto-merge` opt-in and are UNCHANGED byte-for-byte from the pre-#1365 inline form, since [`workflow_scope_blocked_prs`](../../do-work.md#orchestrator-state) and this file's own cross-references key off that exact shape:
