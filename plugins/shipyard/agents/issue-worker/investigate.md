...
Work an **untriaged** issue — one `/do-work`'s label filter would normally drop (carries `needs-triage`, typically authored by a trusted bot like `app/sentry` / `sentry[bot]`) — end-to-end, without assuming a specified bug with inferable acceptance criteria. The goal is a **binary backlog**: every untriaged issue ends this dispatch as either *workable-by-`do-work`* (a PR opened) or *workable-by-human* (`needs-human` applied) or *closed* (confident noise / duplicate) — never left in a permanent third "untriaged" state.
...
#### 4b. Genuinely needs a human → apply `needs-human`, return blocked-style (the `investigated+needs-human` path)
...
```bash
gh issue edit <N> --repo <owner/repo> --add-label needs-human --remove-label needs-triage
...
Routing to the human queue (`needs-human`) rather than guessing.
EOF
)"
```

`needs-human` is the **single human-queue label** the binary-triage model converges on — distinct from `needs-human-review` (which gates an *already-opened external-author PR*). `needs-human` means "this issue needs a person before any PR can be opened." Removing `needs-triage` and adding `needs-human` is what moves the issue out of the permanent-untriaged state into the workable-by-human state.

Return: `investigated+needs-human #<N> (label applied)`.
...
|---|---|---|
| `off` | NEVER auto-close — route to `needs-human` (4b) instead | NEVER auto-close — route to `needs-human` (4b) instead |
...
| Needs a human | `investigated+needs-human #<N> (label applied)` |
...
**`reaped:` is retryable; `blocked:` is deterministic; `investigated+*` are terminal successes.** The orchestrator's reconcile re-enqueues on `reaped:`, tags `blocked:agent` on `blocked:`, and on any `investigated+*` treats the untriaged issue as dispositioned (removed from the untriaged queue) — that's how the backlog converges to binary.
...
- **Don't auto-close an issue you're not confident about.** When in doubt, route to `needs-human` (4b) — an uncertain close drops a real bug. Honor the `triage.auto_close` policy: `off` means NEVER close; `confident-only` means certain-only.
- **Don't apply `needs-human-review` for the human-queue disposition.** That label is the *external-author-PR* gate. The investigate-mode human queue is `needs-human` — a distinct label for "needs a person before any PR exists." (`needs-human-review` is still correct for the 4a fixable path when `originating_author_trust == external`, exactly as in `issue-work`.)
- **Don't leave `needs-triage` on the issue in ANY disposition.** Every terminal path removes it (fix removes it, needs-human removes-and-replaces it, close removes it implicitly). Leaving it on is the permanent-untriaged state this mode exists to eliminate.
