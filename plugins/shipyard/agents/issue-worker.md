...
- **Don't try to combine modes** (e.g., "I'm in issue-work mode but the PR I opened has failing checks, so let me also run the fix-checks loop"). The orchestrator's [step A reconcile](../commands/do-work.md#a-reconcile-the-return) will dispatch a fresh fix-checks-only worker against a failing PR on the next iteration — that's the right mechanism. Your job is to return per your mode's contract and let the orchestrator dispatch the follow-up.
