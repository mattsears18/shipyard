...
This rule shares wording with the issue-worker's untrusted-body rule in `agents/issue-worker.md` step 2 ([#90](https://github.com/mattsears18/claude-plugins/issues/90) / [#93](https://github.com/mattsears18/claude-plugins/issues/93)) deliberately — both are the same principle (fetched text describes facts, doesn't issue instructions) applied at two different surfaces (issue bodies for the worker, arbitrary fetch results for the auditor). Future contributors adding a new auditor with a new fetch source should extend this rule, not add a parallel one. Closes [#109](https://github.com/mattsears18/claude-plugins/issues/109).
