...
- **The happy path never races.** A compliant worker reaches its PR-creation step (issue-work [§5](../../agents/issue-worker/issue-work.md#5-commit--push--pr): `git commit` / `git push` / `gh pr create --label shipyard ...`) **before** its final turn ends — the "Return-contract discipline" rules below already forbid ending a turn with uncommitted or un-PR'd work. By the time the harness's own end-of-subagent auto-commit/push/draft-PR logic would fire, the worktree is already clean and the branch already has an open, `shipyard`-labeled, closing-keyword-linked PR — the native behavior has nothing left to act on and is a no-op.
...
- **Scope of the risk.** This race is specific to the `Agent`-tool subagent mechanism (`background` / `isolation` frontmatter) — i.e., shipyard's legacy `dispatch.substrate: "agent"` path. The default `dispatch.substrate: "workflow"` path (as of [#790](https://github.com/mattsears18/shipyard/issues/790)) dispatches workers via the `Workflow` tool's `agent()` calls, which Claude Code's docs describe as a distinct mechanism with its own per-run budget — not a "subagent" in the `background`/`isolation`-frontmatter sense — so the native auto-commit/push/draft-PR behavior does not apply there, and PR creation stays entirely orchestrator/worker-authored as already documented in [`workflows/README.md`](../../workflows/README.md#worktree-isolation-under-the-workflow-substrate). Since `workflow` is now the built-in default, this residual gap only matters for a repo that has opted back into (or hasn't yet moved off) `dispatch.substrate: "agent"`.

## Return-contract discipline

Every worker mode's last line is the orchestrator's only signal of outcome. The valid terminal strings are mode-specific (see the dispatching prompt for the exact set), but five universal rules apply:
