...
# /my-turn-and-do is the memorable "do my work AND operate the browser" alias —
# now equivalent to a bare /do-work since the operator layer is the default.
# Run --setup first: a self-onboarding preflight for the browser extension,
# gh auth, and site permissions.
...
| `/do-work --operate` (= `/my-turn-and-do`) | Same as a bare `/do-work` — the operator layer is now the default, so `--operate` is a retained no-op / memorable-alias form | yes (autonomous) |
