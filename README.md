...
### Dispatch substrate — Dynamic Workflows

Every `mode:`-driven worker is dispatched through Claude Code's native [Dynamic Workflows](https://code.claude.com/docs/en/workflows) — a deterministic JavaScript orchestration script (`plugins/shipyard/workflows/do-work-dispatch.workflow.js`) rather than prose the orchestrator re-interprets each turn. Because the `Workflow` runtime's `agent()` primitive has no isolation option of its own, the orchestrator pre-provisions each worker's git worktree itself (`git worktree add`) and passes the path as that work unit's `worktreePath` — the worker's first instruction is to `cd` there and verify the anchor before doing anything else. Each dispatch returns a **schema-validated structured result** (`plugins/shipyard/schemas/worker-return.schema.json`) that the orchestrator translates back into the free-text return vocabulary before reconcile, so a malformed or ambiguous return fails loudly at the stage boundary instead of being re-parsed from prose. The `Agent` tool is still used elsewhere — the opt-in verify gate, the epic-decomposition worker, and the read-only scope-preflight / refinement workers — none of which take a `mode:` value.

> **⚠️ Upgrading from ≤ 3.1.x:** the `dispatch.substrate` config knob is **removed as of `4.0.0`** — there is now one substrate, no flag, no fallback. Delete any `dispatch` block from `shipyard.config.json` / `.shipyard/config.local.json` before upgrading: the config schema is `additionalProperties: false`, so a layer that still carries one now **fails validation** (`/shipyard:config validate` reports it; `shipyard-config.sh load` exits 70) instead of being silently ignored.
...
plugins/shipyard/
  ...
  workflows/                   # Dynamic Workflows dispatch substrate (do-work-dispatch.workflow.js) — see
                               #   "Dispatch substrate" above; the only way a mode:-driven worker is dispatched
