...
> **Binary-backlog migration (#515).** The long-term north star is a strictly binary issue backlog: every open issue is either workable-by-`/do-work` (no gate label) or workable-by-human (`needs-human-review` → surfaced by `/my-turn`). Phase 1 of that migration (#515) folded the **inert** design-gate label `needs-design` into `needs-human-review` — `needs-design` was a pure dispatch-exclusion + `/my-turn`-surfacing label with no auto-processing machinery, so collapsing it is a lossless rename. The labels that still carry **live auto-processing / auto-clearing machinery** were intentionally **kept separate** in phase 1 because folding them into the inert `needs-human-review` would silently break that machinery (the same hazard `blocked:agent-soft` documents — never route auto-retryable/auto-processed work to the human queue): `needs-refinement` (auto-processed by `/refine-issues` same session), `blocked:agent-hard` (auto-cleared by the referential `Blocked by #N` sweeps), and the `needs-decomposition`/`tracking` epic-decomposition pair (driven by `/decompose-epic`). `needs-triage` is **kept as a transitional parking label** — its elimination depends on the investigate-then-fix worker mode (#514) routing untriaged/bot issues to investigate-mode instead of parking them, and #514's orchestrator runtime integration is phase-2 (not yet live). Folding the remaining labels is tracked as follow-up issues.

- `needs-refinement` — **generic pipeline gate**: "this issue isn't ready for `/shipyard:do-work` dispatch yet — a refiner needs to process it first." Applied conditionally at intake by `.github/workflows/intake-refinement-gate.yml` (external authors, bodies with `## Open questions` headings, bare one-liners, bot-authored). `/shipyard:refine-issues` branches by source signal: user-feedback gets classify+rewrite, open-questions get resolve-defaults, anything else falls through to `escalate-to-triage` (which swaps `needs-refinement` for `needs-triage`). **Kept separate from `needs-human-review`** (#515): it's auto-processed each session by `/refine-issues`, so folding it into the inert human-gate would strand refinable work in the human queue.
- `needs-human-review` — a human sign-off gate, decoupled from `needs-refinement`: applied by the user-feedback classify+rewrite branch of `/shipyard:refine-issues`, the `external-author-gate.yml` workflow, and `issue-worker.md` step 6 for external-author PRs. The resolve-defaults and escalate-to-triage branches do NOT apply it — trusted-author issues that pass through resolve-defaults become dispatch-eligible immediately. **As of #515 this label also subsumes the former `needs-design` design-gate** — a design-gated issue (blocked on a human design decision before it's agent-workable) now carries `needs-human-review`; `/do-work` excludes it from dispatch and `/my-turn` surfaces it as a human-blocked item, exactly as it did for the standalone `needs-design` label.
